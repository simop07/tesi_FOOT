...
\begin{document}
	...
	\begin{figure}[H]
		...
		\caption{Meccanismi di trascrizione e traduzione grazie ai quali il genotipo (l'insieme delle informazioni ereditarie di un organismo) controlla il fenotipo (le caratteristiche fisiche di un organismo) \cite{hub_scuola,}(cita
...
\begin{figure}[H]
	...
	\begin{itemize}
		...
		\item Camera a emulsione (per frammenti leggeri): dotato di un'accettanza angolare maggiore di $50^\circ$ rispetto alla direzione del fascio, è dotato di una camera a emulsione on la quale poter misurare frammenti leggeri come protoni, deuteroni (nuclei del deuterio), tritoni (nuclei del trizio) e nuclei di \ce{He}.
	\end{itemize}
	
	\subsection{Setup elettronico}
	Il setup elettronico di FOOT è costituito da vari sub-detector con in quali è possibile attuare la ricostruzione isotopica dei frammenti prodotti e l'individuazione della loro carica (si veda \hyperref[fig:electronic_setup]{Fig. 2.1}). Ciò avviene tramite la misura di quantità di moto, energia cinetica e tempo di volo (Time Of Flight, TOF) dei frammenti nucleari \footcite{foot_cdr}. Il rilascio di energia dei frammenti per unità di distanza percorsa $\frac{dE}{dx}$ viene misurato due volte analizzando l'energia depositata $\Delta E$ dai frammenti all'interno dell'apparato di misura. La carica delle particelle viene identificata combinando le misure di $\Delta E$ con quelle del TOF o dell'energia cinetica, mentre la massa a riposo $m$ dei frammenti viene estratta dalle misure di momento $p$ ed energia cinetica $E_k$ attraverso le seguenti relazioni relativistiche:
	\begin{subequations}
		\begin{align}
			\label{eq:momentum}
			p=mc\beta\gamma\\
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Schematizzazione dell'upper region (cita
			%pdf materiale - Seminario Dong
			).}
	\end{figure}
...
%TRACKING REGION
%quello che abbiamo dopo è un sistema di tracciamento, che serve a 2 cose:
%1) per ogni traccia prodotta dobbiamo ricostruire la traiettoria di questa traccia.
%2) ci sono due campi magnetici che deflettono le partielle cariche, per misurare l'impulso. e una particella carica entra in un campo magnetico perpendicolare alla sua velocità, questa sente una forza che non aumenta la velocità della particella (è una forza centripeta). da qui si ricava mv (magari puoi scrivere il calcolo) -> misurando la deflessione si misura l'impulso. si fanno due deflessioni (2 campi magnetici) per avere misure più precise.
%
%il sistema di tracciamento è dato da un vertex, 4 piani costituiti dai fotomultiplyer (2 cm*2 cm). è fatto di tanti pixel che rileva il passaggio della particella. per ogni traccia abbiamo almeno 4 punti, e in teoria la sua traiettoria è dritta perché incontra solo successivamente il primo campo magnetico (vedi disegno). Dopo, la particella va nel campo magnetico, e viene curvata (e lì non abbiamo rivelatori) e poi ve ne sono altri 2 di piani (sostanzialmente uguali a quelli del vertex), in cui si studia la direzione di uscita. da queste direzioni che vengono rilevate, pre e dopo campo magnetico, si riesce a comprendere il raggio di curvatura della particella. poi si usa un MSD, non fatto a pixel come gli altri due precedenti, ma è un tracciatore fatto di strisce orizzontali e verticali sovrapposte, su cui si va a capire il punto per cui la particella passa. in realtà non riusciamo a capire proprio bene il punto, perché abbaimo strisce orizzontali e verticali che possono generare ambiguità. per capire quali sono quelle gisute, si parla di studio del FONDO COMBINATORIALE: in pratica si studia la potenza del segnale elettrico, e si fanno gli accoppiamenti in base alla potenza del segnale elettrico rilasciato.
%
%NOTA: nell'MSD abbiamo strisce orizzontali e verticali per ciascuno dei tre piani. inoltre, abbiamo che, per esempio, una particella potrebbe attivare più di un pixel. per ricostruire sia la traiettoria che il momento, si fa una sorta di indagine a ritroso, cioè si osservano gli ultimi segnali attivati da una particella, e poi si giunge a mano a mano al rilevatore iniziale, questo è un KALMAN FILTER. nota che a pag 20 c'è la formula che permette di ottenere l'impulso a partire dal raggio di curvatura dei frammenti. dipende chiaramente dal campo magnetico, dalla costante 0.3 legata alle unità di misura che si usano e dal raggio.
