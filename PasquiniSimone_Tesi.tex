...
\begin{document}
	...
	Quando i fotoni attraversano la materia subiscono un processo di assorbimento\footnote{L'assorbimento del numero di fotoni $N(x)$ alla profondità $x$ coincide con un'attenuazione dell'intensità $I(x)$ del fascio essendo quest'ultima pari al numero di fotoni per unità di superficie $A$ per unità di tempo $t$: $I(x)=\frac{N(x)}{At}$.} descritto dalla legge di attenuazione di Lambert:
	\begin{equation}
		N(x)=N_0e^{-\mu x}
		\label{eq:lambert}
	\end{equation}
	...
	), e $E_b$ è l'energia di legame del fotoelettrone, che corrisponde all'energia che occorre fornire all'elettrone per slegarlo dall'atomo di metallo entro cui è contenuto (infatti $E_b$ è anche chiamato lavoro di estrazione). Il fotoelettrone, avendo liberato un delle shell più interne agli atomi di metallo, genera una lacuna. Tale "posto vacante" viene successivamente riempito dagli elettroni che appartengono agli orbitali più esterni: a tale salto orbitale corrisponde una grande emissione di energia che in alcuni casi viene assorbita da altri elettroni (è il caso degli elettroni di Auger\footnote{Un elettrone di Auger, appartenendo all'orbitale più esterno dell'atomo, grazie all'energia ricevuta riesce ad essere liberato completamente.}), in altri può generare un raggio X, come mostrato in \hyperref[fig:fotoelettrico]{Fig. 1.26}.
	...
	\begin{equation}
		...
		\label{eq:sigma_fe1}
	\end{equation}
	sia in funzione dei numeri atomico $Z$ e di massa $A$ del bersaglio:
	\begin{equation}
	...
	La produzione di coppia è un processo di interazione elettromagnetica in cui un fotone, dotato di sufficiente energia ($E_\gamma>2m_e=1.022\mbox{ MeV}$), trasforma tutta la sua energia in massa producendo un elettrone e un positrone secondo il diagramma di Feynman mostrato in \hyperref[fig:feynman]{Fig. 1.28}:
	\begin{figure}
		\centering
		\begin{tikzpicture}
			\begin{feynman}[large]
	...
	\end{itemize}
	L'interazione elettromagnetica delle particelle cariche con gli elettroni atomici domina rispetto alle interazioni nucleari per via delle dimensioni lineari dell'atomo e del suo nucleo. Infatti, essendo l'atomo è $10^4$ volte più grande (linearmente) del nucleo, si ha che la sezione d'urto dei processi atomici $\sigma_{atom}$ è più grande di un fattore $10^8$ rispetto alla sezione d'urto che descrive le interazioni nucleari $\sigma_{nucl}$; in particolare, i valori delle sezioni d'urto sono pari a $\sigma_{atom}=100\mbox{ Mb}$ e $\sigma_{nucl}=1\mbox{ b}$. A causa del grande valore di $\sigma_{atom}$ le particelle cariche effettuano moltissime collisioni con gli elettroni atomici, che renderebbe vana una trattazione analitica basata sul calcolo delle sezioni d'urto. Si procede quindi con un approccio energetico in cui si analizza la quantità di energia ceduta dalle particelle cariche durante il loro moto all'interno della materia.
	
	Quando una particella carica si muove in un mezzo materiale causa processi di ionizzazione dei suoi atomi dovuti alla forza che esercita sui loro elettroni (cita
	%christoduides libro relatività
	...
	\begin{table}[H]
		\begin{minipage}{\textwidth}
			...
			\end{tabular}
		\end{minipage}
		\caption{Tabella riassuntiva dei parametri della Bethe-Bloch suddivisi in proprietà del mezzo, costanti, caratteristiche del fascio e correzioni.}
		\label{tab:bethe_bloch}
	\end{table}
	
%	bethe bloch:
%	descrive la perdita di energia lungo il cammino, dx è il cammino della particella. il meno sta per spiegare il fatto che se va avanti la particella perde energia.
%	- Z maiuscolo \rho A è relativo al bersaglio (il corpo umano). Z/A nel nostro casoè un numero vicino a 0.5, visto che il numero di protoni è sempre circa la metà di quello dei nucleoni; rho è la densità del materiale.
%	- le quantità blu dipendono dal nosto fascio, z sarebbe la carica del fascio, beta è la velocità del fascio. (dipendenze quadratiche -> molto importanti.). NOTA BENE: quando la velocità è piccola, ossia l'energia cinetica è piccola, dE/dx è piccolo; vi sono centinaua di interazioni che la particella fa, in ciascuna delle quali la particella rilascia energia poco per volta, fin quando alla fine del cammino c'è il massimo del rlascio di energia. Inoltre vi è anche un termine col logaritmo è legato al potenziale di ionizzazione. poi ci sono dei termini di correzione, quelli per alte e base energie. siamo legati molto al coefficiente C, quello importante per basse energie.
%	
%	questa è una legge statistica (vedi relaitivtà, è un dE/dx medio)
%	esempio: 1 mld di particelle protoniche non si generano tutte allo stesso punto, ma ci sarà una variazione statistica. abbiamo una distribuzione di landau, una gaussiana con una coda (vedi slides).
%	
%	la distribuzione della bethe bloch (con segno invertito) plottata rispetto all'energia. per energia molto alta, dE/dx è circa costante, (il range rosso è legato al nostro caso, quello di bassa energia). quando l'energia inizia a calare, abbiamo una salita che va cmoe 1/beta^2. apparenrtemente c'è un problema: quando la particella del fascio ha energia 0, ci aspettiamo che anche il de/dx sia 0, cosa che non sembra accadere in questo caso guardando il grafico. proprio per questo motivo vanno inserite delle correzioni (shell correction col fattore C): la formula iniziale senza il fattore di correzione è stata scritta pensando al fatto che quando il protone va a incidere sull'elettrone, quest'ultimo stia fermo; ma il problema è che se l'energia cinetica della particella incidente è molto bassa, non si può più trascurare il moto dell'elettrone attorno al nucleo, per questo motivo viene agginuto il termine di correzione C.
%	
%	- poi dalle slide si vede che aumentando lo z aumenta il dE/dx perché va come z^2.
%	
%	
