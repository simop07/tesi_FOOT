...
\begin{document}
	...
	\`E possibile suddividere l'andamento della Bethe-Bloch (mostrato in \hyperref[fig:bethe_bloch]{Fig. 1.32}) in varie regioni.
	\begin{itemize}
		...
		\item Regione relativistica: ad alte energie ($\beta\gamma>4$) si osserva una risalita logaritmica del potere frenante dovuta alla concentrazione delle linee di campo elettrico nella direzione ortogonale alla sua velocità, un effetto puramente relativistico espresso dal termine logaritmico in \hyperref[eq:bethe_bloch]{Eq. 1.35} (cita
		%christoduides libro relatività, capitolo eletromagnetismo
		).
	\end{itemize}
	
	Suddette regioni non includono l'andamento della Bethe-Bloch per energie molto alte ed energie molto basse
	+ barkas effect
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{bethe_bloch.png}
		\caption{Valore assoluto del potere frenante per $\mu^+$ in rame in funzione di $\beta\gamma=p/Mc$ dove $p$ è il modulo della quantità di moto relativistica del $\mu^+$. Le bande verticali delimitano varie regioni di approssimazione dovute a successivi miglioramenti della Bethe-Bloch (cita
