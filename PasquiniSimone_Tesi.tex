...
%	\begin{figure}[H]
%		\centering
%		\includegraphics[width=0.9\linewidth]{bragg_peak_energies.jpg}
%		\caption{La curva nera individua il rilascio di energia in funzione della profondità per fasci protonici la cui energia varia da $60\mbox{ MeV}$ a $200\mbox{ MeV}$ (cita
...
%	\end{figure}
	...
	Tornando alla \hyperref[fig:photon]{Fig. 1.17} è evidente che il rilascio di energia per il fascio di atomi di carbonio ($C$), a differenza del caso dei protoni ($p$), prosegue anche dopo il BP. Osservando l'andamento quadratico $\propto\frac{z^2}{\beta^2}$ della \hyperref[eq:bethe_bloch]{Eq. 1.35}, pur essendo $z_p<z_C$, si ha che $\frac{dE_p}{dx}>\frac{dE_C}{dx}$ in quanto le energie cinetiche dei carboni sono maggiori di quelle dei protoni.\footnote{Solitamente le energie cinetiche $K$ sono $K_p=250 \mbox{ MeV}$ e $K_C=4800\mbox{ MeV}=12\mbox{ u}\cdot400\mbox{ MeV/u}$.} In effetti il profilo di dose rilasciato dai fasci di $C$ è maggiore di quello dei $p$, ma ciò non spiegherebbe comunque il rilascio di energia dopo il BP. Tale effetto si può comprendere solo analizzando le interazioni nucleari delle due particelle.
	
	\subsection{Interazioni nucleari}
	
	
%	ci sono differenze tra protoni e carbonio (la BB va come Z^2); la differenza maggiore forse risiede quando si va in profondità: il carbnio ha energia sufficiente per andare oltre il picco di Bragg, cosa che non accade nei protoni. Non c'è effetto che è possibile spiegare con le consocenze che abbiamo studiato fin ora: risiedono nelle interazioni nucleari. quello che succede è che quando entriamo con un fascio, abbiamo che il fascio può fare interazione nucleare come a pag 51. le interazioni possono spaccare il nucleo, ma possono spaccare il protone? no, almeno non a queste energie. possiamo spaccare solo il nucleo. cosa succede se lo spacchiamo? a queste energie si conserva il numero di neutroni e il numero di protoni. spaccando il carbonio (ad esempio) che cosa otteniamo? altri nuclei che hanno una carica inferiore. se abbiamo altri  nuclei con carica inferiore (z minore) al nostro carbonio -> abbiamo un minore dE/dX secondo la bethe bloch, ossia questi prodotti perdono meno energia -> significa che questi prodotti vanno più avanti. c'è un però: il carbonio e l'idrogeno, ossia i bersagli più comuni del nostro corpo, che possiamo considerare fermi, sono molto più grandi del protone che ci sbatte contro, per questo l'energia acquisita dal bersaglio è pochissima se non nulla, per questo nel caso in cui il proiettile è il protone, abbiamo che l'energia rilasciata dopo la BB è sostanzialmente nulla. Cosa diversa avviene nel caso in cui il proiettile è un carbonio. se si spacca, si creano nuclei più piccoli con z^2 minore, e quindi capaci di disperdere meno energia (questo come prima); laa differenza qui sta nel fatto che la frammentazione del proiettile fa si che i frammenti vengono prodotti alla stessa beta del fascio, circa, quindi la loro corsa non ferma, anche se magari è rallentata dal fatto che z dei prodotti è comunque minore di quello dei reagenti. in sostanza, i nuclei del corpo umano, una volta spaccati, non fanno strada, a differenza dei frammenti dei proiettili.
%		
%		pag 39: il grafico nero rappresenta i dati sperimentali con fascio di carbonio a 400 MeV. il picco si ha attorno a 27 cm. man mano che il fascio a carbonio entra, le partielle del fascio diminuiscono perché il carbonio va incontro a frammentazione, quindi sostanzialmente è come se il fascio si stesse trasformando in frammenti diversi dalle particelle primarie iniziali (carbonio). l'oggetto in nero è la soma delle due curve rossa e blu. i frammenti secondari sono quelli che vanno oltre il punto che avevamo stabilito per il carbonio. questo è un problema.
%		se la bethe bloch va come z^2, perché non prendo ferro, piombo? perché le reazioni nucleari diventano predominanti e vi sarebbe un rilascio di energia troppo eccessivo a basse profondità tanta energia all'inizio. si pensa che oltre l'ossigeno non ha più senso andare.
%	
