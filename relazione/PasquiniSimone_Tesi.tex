...
%	\end{itemize}
	...
	\begin{table}[H]
		\begin{minipage}{\textwidth}
			\centering
			\begin{tabular}{ |M{2.0cm}||M{1.5cm}|M{1.5cm}|M{1.5cm}|M{1.5cm}|M{1.5cm}| }
				\hline
				...
				\hline
				\hline
				Frammento &\ce{^7_{3}Li}&\ce{^7_{4}Be}&\ce{^{9}_{4}Be}&\ce{^{10}_{5}B}&\ce{^{12}_{6}C}\\
				\hline
				$z$ & $3$& $4$& $4$& $5$& $6$\\
				\hline
				$A$ & $7$& $7$ &$9$ &$10$& $12$\\
	...
	\begin{table}[H]
		\begin{minipage}{\textwidth}
			...
			\end{tabular}
		\end{minipage}
		\caption{Ricostruzione delle migliori stime dei numeri atomici $z\le3$, le cui incertezze sono pari alla $\sigma$ dei fit gaussiani evidenziati in \hyperref[fig:atomic_numbers]{Fig. 3.1}.}
	...
	\begin{table}[H]
		\begin{minipage}{\textwidth}
			...
			\end{tabular}
		\end{minipage}
		\caption{Ricostruzione delle migliori stime dei numeri atomici $z>3$, le cui incertezze sono pari alla $\sigma$ dei fit gaussiani evidenziati in \hyperref[fig:atomic_numbers]{Fig. 3.1}.}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.\linewidth]{c_Berillum.pdf}
	...
	Procedendo nelle modalità descritte in \hyperref[sec:selection_number_of_mass]{Sez. 3.3.2}, è possibile ricostruire i numeri di massa $A_1$, $A_2$ e $A_3$ di tutti gli isotopi dei frammenti maggiormente prodotti nella simulazione, al fine di completare l'identificazione univoca degli stessi. Prima di procedere con l'analisi, si osserva che le distribuzioni degli istogrammi di tipo $1$ e di tipo $2$ di tutti i frammenti considerati sono piuttosto simili. Sebbene questo sia un risultato quasi scontato vista la somiglianza dei metodi di taglio utilizzati nelle \hyperref[fig:a1_cut]{Fig. 3.5}, \hyperref[fig:a2_cut]{Fig. 3.6} e \hyperref[fig:a3_cut]{Fig. 3.7}, conferma l'assenza di errori sistematici nel programma di analisi dati. Pertanto, si prosegue con l'identificazione dei numeri di massa utilizzando solo istogrammi di tipo $1$.
	
	Per ottenere le misure di $A_1$, $A_2$ e $A_3$ dei frammenti in \hyperref[tab:fragments]{Tab. 3.1}, si proiettano su istogrammi monodimensionali le occorrenze tagliate dalle distribuzioni in \hyperref[fig:a1_cut]{Fig. 3.5}, \hyperref[fig:a2_cut]{Fig. 3.6} e \hyperref[fig:a3_cut]{Fig. 3.7}. In seguito, si effettuano i fit gaussiani sugli istogrammi monodimensionali, ottenendo per $A_1$, $A_2$ e $A_3$ rispettivamente gli istogrammi in \hyperref[fig:a1_fragments_final]{Fig. 3.9}, \hyperref[fig:a2_fragments_final]{Fig. 3.10} e \hyperref[fig:a3_fragments_final]{Fig. 3.11}. Estrapolando i dati di fit si ottengono le migliori stime e le relative incertezze di $A_1$, $A_2$ e $A_3$, riassunte in \hyperref[tab:mass_numbers]{Tab. 3.4}. Infine, in \hyperref[fig:number_mass_resolution]{Fig. 3.12} sono rappresentate le risoluzioni percentuali di $A_1$, $A_2$ e $A_3$.
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.\linewidth]{c_Total_black1.pdf}
		\caption{Distribuzioni del numero di massa $A_1$ e relativi fit.}
		\label{fig:a1_fragments_final}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.\linewidth]{c_Total_black2.pdf}
		\caption{Distribuzioni del numero di massa $A_2$ e relativi fit.}
		\label{fig:a2_fragments_final}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.\linewidth]{c_Total_black3.pdf}
	...
	\begin{table}[H]
		\begin{minipage}{\textwidth}
			...
			\begin{tabular}{ |M{2.0cm}|M{3.0cm}|M{3.0cm}|M{3.0cm}| }
				...
				\ce{^{2}_{1}H}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^{3}_{2}He}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^{4}_{2}He}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^6_{3}Li}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^7_{3}Li}& $\pm$& $\pm$ &$\pm$\\
				...
				\ce{^7_{4}Be}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^{9}_{4}Be}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^{10}_{5}B}& $\pm$& $\pm$ &$\pm$\\
				\hline
				\ce{^{12}_{6}C}& $\pm$& $\pm$ &$\pm$\\
				\hline
			\end{tabular}
		\end{minipage}
		\caption{Ricostruzione delle migliori stime dei numeri di massa $A_1$, $A_2$ e $A_3$ ottenuti dopo l'applicazione delle funzioni di taglio, le cui incertezze sono pari alla $\sigma$ dei fit gaussiani evidenziati rispettivamente \hyperref[fig:a1_fragments_final]{Fig. 3.9}, \hyperref[fig:a2_fragments_final]{Fig. 3.10} e \hyperref[fig:a3_fragments_final]{Fig. 3.11}.}
		\label{tab:mass_numbers}
	\end{table}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.\linewidth]{c_multigr_final.pdf}
		\caption{.}
		\label{fig:number_mass_resolution}
	\end{figure}
	
	
	ANALISI DEGLI ANDAMENTI DELLA RISOLUZIONE.
	
	Tali misure sono caratterizzate da una risoluzione che va dal $5.3\%$ dell'\ce{_{1}H} al $2.3\%$ del \ce{_{6}C} e presentano uno shift rispetto ai valori attesi. Una spiegazione plausibile dello shift potrebbe risiedere nelle già citate perdite di energia dovute ai processi di ionizzazione che precedono il TW e alla produzione di neutroni.
	...
	\markboth{CONCLUSIONI (TBD)}{CONCLUSIONI (TBD)}
